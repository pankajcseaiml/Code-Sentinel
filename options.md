...
Options:
  -c, --config <key=value>
          Override a configuration value that would otherwise be loaded from the standard Opencode configuration (`opencode.json` discovered via XDG paths). Use a dotted path (`foo.bar.baz`) to override nested values. The `value` portion is parsed as JSON.
